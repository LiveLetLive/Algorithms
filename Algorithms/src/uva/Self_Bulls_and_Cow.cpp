...
#ifdef SELF_BULLS_AND_COW
READ_INPUT(SELF_BULLS_AND_COW)
...
private:
	void guess()
	{
		...
		while( true )
		{
			...
			if( gs.empty() ) 
			{ 
				cout << endl << "Something went wrong with the scoring..." << endl << "Cannot find an answer!" << endl; 
				return; 
			}

			if( scoreIt( gs, res ) ) 
			{ 
				cout << endl << "I found the secret number!" << endl << "It is: " << gs << endl; 
				return; 
			}
